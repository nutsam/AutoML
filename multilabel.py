...
class MultilabelPredictor:
    """
    Tabular Predictor for predicting multiple columns in a table.
    ...
    Args:
        ...
        path (str): Path to the directory where models and intermediate outputs should be saved.
            If unspecified, a time-stamped folder called "AutogluonModels/ag-[TIMESTAMP]" will be created in the working directory to store all models.
            Note: To call `fit()` twice and save all results of each fit, you must specify different `path` locations or don't specify `path` at all.
            Otherwise, files from the first `fit()` will be overwritten by the second `fit()`.
        ...
        consider_labels_correlation (bool): Whether the predictions of multiple labels should account for label correlations or predict each label independently of the others.
            If True, the ordering of `labels` may affect resulting accuracy as each label is predicted conditional on the previous labels appearing earlier in this list (i.e., in an auto-regressive fashion).
            Set to False if during inference you may want to individually use just the ith TabularPredictor without predicting all the other labels.
        **kwargs: Additional arguments passed into the initialization of each TabularPredictor.
    """
    ...
    def fit(self, train_data, tuning_data=None, **kwargs):
        ...
        for i in range(len(self.labels)):
            ...
            if not self.consider_labels_correlation:
                labels_to_drop = [_label for _label in self.labels if _label != label]
            else:
    ...
    def predict(self, data, **kwargs):
        """
        Returns a DataFrame with label columns containing predictions for each label.
    ...
    def _predict(self, data, as_proba=False, **kwargs):
        ...
        for label in self.labels:
            ...
            if as_proba:
                ...
                predproba_dict[label] = predictor.predict_proba(data, as_multiclass=False, **kwargs)
            data[label] = predictor.predict(data, **kwargs)
        if not as_proba:
